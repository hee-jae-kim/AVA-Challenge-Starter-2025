...
def loss_function(X, Y_r, Y, mu, logvar):
    ...
    # V_loss = torch.abs((Y_w - Y_waypoint)).sum() / Y_w.shape[1]
...
def train(epoch):
    ...
    generator = list(dataset.sampling_generator(num_samples=cfg.num_vae_data_sample, batch_size=cfg.batch_size))  

    for motion3d_np, text_embd_np in generator:

        # motion: Seq_len x Batch_size x Joint_dim
        motion3d_np = motion3d_np.reshape(motion3d_np.shape[0], motion3d_np.shape[1], -1)
        motion3d = tensor(motion3d_np, device=device, dtype=dtype).permute(1, 0, 2).contiguous() 
        
        # text:  Batch_size x Joint_dim
        text_embd = tensor(text_embd_np, device=device, dtype=dtype).contiguous()

        X = motion3d[:t_his]
        Y = motion3d[t_his:]
        
        Y_r, mu, logvar = model(X, Y, text_embd)
